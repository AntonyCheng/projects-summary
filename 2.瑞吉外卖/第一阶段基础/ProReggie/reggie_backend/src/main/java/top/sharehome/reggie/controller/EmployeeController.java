...
public class EmployeeController {
     ...
     * @param request  这里需要往前端session中存入登录用户的id
     * @return 返回统一包装后的R类型
     */
    @PostMapping("/login")
    public R<Employee> login(@RequestBody Employee employee, HttpServletRequest request) {
        ...
        request.getSession().setAttribute("loginUserId", employeeInBackend.getId());
     ...
     * 用户退出功能
     *
     * @param request 退出时需要删除掉session中用户登陆的id
     * @return 返回退出的日志信息
     */
    @PostMapping("/logout")
     ...
     * @param request 修改完之后需要把session登陆状态给删除掉，以防止用户没有重新登录就继续操作
     * @return 返回修改密码成功与否的信息
     */
    @PostMapping("/password")
    public R<String> updatePassword(@RequestBody Map<String, Object> params, HttpServletRequest request) {
     ...
     */
    @DeleteMapping("")
    public R<String> delete(@RequestBody Employee employee, HttpServletRequest request) {
        Long loginUserId = (Long) request.getSession().getAttribute("loginUserId");
